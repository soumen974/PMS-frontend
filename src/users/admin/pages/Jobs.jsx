  ...
  const handleFormSubmit = async () => {
    try {
      ...
      const response = await axios.put(
        `/api/admin/updateJob/${jobId}/${companyId}`,
        {
      ...
      if (response.status === 200) {
        ...
        navigate("/Admin/jobs"); // Navigate to "/jobs" after successful update
      } else {
            ...
            {jobs.map((job) => (
              <tr key={job.id}>
                ...
                <td>
                  ...
                  <button
                    className="btn btn-sm bg-black ms-2 text-white"
                    onClick={handleClick}
                  >
                    Delete
                  </button>
